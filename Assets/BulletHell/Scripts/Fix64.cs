...
public partial struct Fix64 : IEquatable<Fix64>, IComparable<Fix64> {
   ...
   public static explicit operator Fix64(long value) {
    ...
    {
        // 幂运算（整数指数）：使用分治递归（Exponentiation by Squaring）。
        // 注意：这里的实现假设 y 为正整数（y==1 为递归终止条件），不支持 y<=0。
        if (y == 1) return x;
    ...
    {
        // 平方根：牛顿迭代（Babylonian method）。
        // numberIterations 越大越精确，但也更耗时；该方法对不同数量级输入需要不同迭代次数。
        if (f.RawValue < 0)
        {
    ...
    {
        // 根据输入 raw 值大小选择迭代次数，避免在较大数值时收敛不足。
        // 这些阈值是经验/调参结果，用于折中性能与精度。
    ...
    {
        // 正弦：将角度归一化到 [0, 2π]，再映射到 [0, 360] 的“度”刻度，
        // 最终通过查表 + 线性插值计算。
        // 这里 i 的单位与 PI 常量一致（Fix64 内部约定的角度标度），并非标准弧度 double。
        Fix64 j = (Fix64)0;
        for (; i < Fix64.Zero; i += Fix64.FromRaw(PiTimes2));
        if (i > Fix64.FromRaw(PiTimes2))
            i %= Fix64.FromRaw(PiTimes2);

        // 将 [0, 2π] 映射到 [0, 360]，k 为“度”的整数部分，j 为 0~9 的小数刻度（用于插值）。
    ...
    {
        // 查表并做线性插值：SIN_TABLE 以 0~90 度为索引，表值缩放为 0~ONE。
        // j 是 0~9 之间的小数刻度（把 1 度划分为 10 份）用于插值。
    ...
    {
        // tan(x) = sin(x) / cos(x)
        // 注意：当 cos(x) 接近 0 时会发生数值放大甚至除零异常。
        return Sin(i) / Cos(i);
    }

    public static Fix64 Asin(Fix64 F)
    {
        // 反正弦：对输入做符号拆分，并通过多项式近似 + sqrt 的组合实现。
        // 输入范围要求：F ∈ [-1, 1]（Fix64.One 表示 1）。超出范围抛异常。
        bool isNegative = F < 0;
        F = Abs(F);
    ...
    {
        // atan2(y, x)：根据 x/y 的符号确定结果所在象限，处理 x==0 的边界。
        // 返回值与 PI 常量同单位。
